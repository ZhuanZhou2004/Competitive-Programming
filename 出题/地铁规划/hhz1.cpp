...
int n, r;
int S[200010], sta[200010], pos[200010], tag[200010];
int top, tops;
...
inline void real_pop(int l, vector < int > a, int x) {
    ...
    if (a.size() - 1) S[++tops] = a.size() - 1, tag[tops] = 1;
    for (auto i : a) if (i != x) _merge(i), pos[i] = tops;
}
inline vector < int > real_sort(int len) {
    vector < int > ans;
    if (tag[tops]) {
...
inline void pop(int x) {
    ...
    S[tops] += qwqdel + 1;
    vector < int > b = real_sort(min(S[tops], sum + 1 - (int)a.size()));
